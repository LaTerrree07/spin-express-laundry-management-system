<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-2xl font-semibold text-[#1E293B]">Customer Transactions Report</h2>
    ...
    <div class="space-y-6">
        <div class="rounded-2xl border border-[#D6DEE8] bg-white p-4 shadow-sm">
            <form method="GET" action="{{ route('reports.customer-transactions') }}" class="flex flex-col gap-3 sm:flex-row">
                <div class="relative w-full">
                    ...
                    <input
                        ...
                        name="search"
                        value="{{ $search }}"
                        placeholder="Search customer, transaction, service, or status"
                ...
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('reports.customer-transactions') }}" class="inline-flex items-center justify-center gap-2 rounded-2xl border border-[#D6DEE8] bg-white px-4 py-3 text-sm font-medium text-[#1E293B] transition hover:bg-slate-50">
        ...
        <div class="overflow-hidden rounded-2xl border border-[#D6DEE8] bg-white shadow-sm">
                ...
                <table class="min-w-full divide-y divide-[#D6DEE8]">
                    <thead class="bg-[#EEF4FA]">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-[#0F213E]">Contact</th>
                    ...
                    <tbody class="divide-y divide-[#D6DEE8] bg-white">
                        @forelse ($customerTransactions as $row)
                            <tr class="hover:bg-[#F8FBFF]">
                                <td class="px-4 py-4 text-sm font-medium text-[#1E293B]">{{ $row->customer_name }}</td>
                                <td class="px-4 py-4 text-sm text-[#64748B]">{{ $row->contact_number }}</td>
                                <td class="px-4 py-4 text-sm text-[#64748B]">{{ $row->transaction_id ? '#'.$row->transaction_id : '—' }}</td>
                                ...
                                <td class="px-4 py-4 text-sm text-[#64748B]">{{ $row->total_amount ? '₱'.number_format((float) $row->total_amount, 2) : '—' }}</td>
                            </tr>
                        @empty
                            <tr>
            ...
            @include('layouts.partials.table-pagination', ['paginator' => $customerTransactions])
        </div>
    </div>
</x-app-layout>